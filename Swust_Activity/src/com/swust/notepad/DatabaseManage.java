...
public class DatabaseManage {
	...
	public Cursor selectById(int id){
		...
		Cursor cursor = null;
		try{
			String sql = "select * from Notepad where _id='" + id +"'";
			cursor = mSQLiteDatabase.rawQuery(sql, null);
		}catch(Exception ex){
			ex.printStackTrace();
			cursor = null;
	...
	public long insert(String title, String content){
		...
		try{
			...
			l = mSQLiteDatabase.insert("Notepad", null, cv);
	...
	public int delete(long id){
		int affect = 0;
		try{
			affect = mSQLiteDatabase.delete("Notepad", "_id=?", new String[]{id+""});
		}catch(Exception ex){
			ex.printStackTrace();
			affect = -1;
		}
	...
	public int update(int id, String title, String content){
		...
		try{
			...
			cv.put("content", content);
			String w[] = {id+""};
			affect = mSQLiteDatabase.update("Notepad", cv, "_id=?", w);
		}catch(Exception ex){
			ex.printStackTrace();
			affect = -1;
		}
